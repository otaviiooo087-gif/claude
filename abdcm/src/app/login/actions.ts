...
export async function entrar(_estado: EstadoLogin, form: FormData): Promise<EstadoLogin> {
  ...
  const user = banco().users.find((u) => u.email === parsed.data.email.toLowerCase().trim())
  ...
  if (!user || !user.isActive || !conferirSenha(parsed.data.senha, user.passwordHash)) {
  ...
  auditar({
    tenantId: user.tenantId, atorUserId: user.id, atorNome: user.nome, acao: 'sessao.login',
